...
class TarafeNaoEncontrada(Exception):
    pass
...
class Projeto:
    ...
    def procurar(self, descricao):
        # o try é usado quando tem um trecho que pode dar problema
        try:
            # Possível IndexError
            return [tarefa for tarefa in self.tarefas
                    if tarefa.descricao == descricao][0]
        except IndexError as e:
            raise TarafeNaoEncontrada(str(e))
...
def main():
    ...
    except TarafeNaoEncontrada as e:
    ...
    mercado.add('Carne')
    mercado.add('Tomate', datetime.now() + timedelta(days=3, minutes=5))
    print(mercado)
